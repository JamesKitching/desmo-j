...
import desmoj.extensions.space3D.Length;
	...
	protected SpatialObject createSpatialObject(Element spatialObject) {
		...
		if(type.equals("TruckQueue")){
			...
			SpatialProcessQueue<Truck> queue = new SpatialProcessQueue<Truck>(this._model,spatialObject.getAttribute("Name"),
						type,showInReport,showInTrace,new Length(x),
						new Length(y),new Length(z));
			...
			for(int i=0;i<entryPoints.getLength();i++){
				...
				queue.addEntryPoint(entryPoint.getAttribute("name"),
						new Length(new Double(entryPoint.getAttribute("x"))),
						new Length(new Double(entryPoint.getAttribute("y"))),
						new Length(new Double(entryPoint.getAttribute("z"))));
			}
			...
			for(int i=0;i<exitPoints.getLength();i++){
				...
				queue.addExitPoint(exitPoint.getAttribute("name"),
						new Length(new Double(exitPoint.getAttribute("x"))),
						new Length(new Double(exitPoint.getAttribute("y"))),
						new Length(new Double(exitPoint.getAttribute("z"))));
			}
