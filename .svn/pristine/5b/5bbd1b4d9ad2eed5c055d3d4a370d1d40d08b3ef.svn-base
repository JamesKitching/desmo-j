 ...
 * 
 * @modifier A new method with signatur restSettingsPanel()
 * @author Xiufeng Li, Binhua Han, Rene Wecker 
 * @date 30.03.2011
 */

public class ExperimentStarterApplicationModifier extends JFrame implements
		ExperimentStarterGUI
{
	...
	private final static String TITLE = "DESMO-J "
			+ Experiment.getDesmoJVersion() + " Experiment Starter";

	/**
	 * Creates a new experiment starter application without loading a model
	 */
	public ExperimentStarterApplicationModifier()
	{
		this(null, null);
	}
	 ...
	 */
	public ExperimentStarterApplicationModifier(Class modelClass,
	 ...
	 */
	public ExperimentStarterApplicationModifier(String confFileName)
	{
		try
		{
			jbInit();
		...
		startButton.addActionListener(new java.awt.event.ActionListener()
		{
			public void actionPerformed(ActionEvent e)
			{
				if (experimentStarter.runsBatch)
					...
					try
					{
						experimentStarter.startExperiment(settingsPanel);
	 ...
	 *            AccessPointTableModel: the access points for the model
	 *            settings
	 * @param expAccessPoints
	 *            AccessPointTableModel: the access points for the experiment
	 *            settings
	 * @param modelName
	...
	public void resetGUI(String filename, boolean runsBatch,
			...
			AccessPointTableModel expAccessPoints, String modelName)
	{
	...
	public static void main(String argv[])
	{
		ExperimentStarterApplication gui = new ExperimentStarterApplication(
				BalticSeaModel.class, ExperimentRunner.class);
		gui.setVisible(true);
	}
	
	
	/** Reset alle Komponenten von dem Seetingspanel. */
