...
//	}
	 ...
	 */
	public static Movement configureMovement(Movement movement, double totalDistance, double initialSpeed, double maxSpeed,
		...
		if(fastestPossibleMovement.getTotalDuration().getTimeAsDouble(TimeUnit.SECONDS)<totalDuration){
			...
			double breakingFromInitialSpeedToZeroDuration = getDurationOfSpeedBoundedAcceleration(initialSpeed, 0.0d, dec);
			...
			double initialSpeedToDestinyDistance = (initialSpeed*initialSpeedDuration)+getDistanceOfSpeedBoundedAcceleration(initialSpeed, 0.0d, dec);
			...
			if (initialSpeedToDestinyDistance<totalDistance){
				...
				if(squared<0){
					throw new IllegalArgumentException("The squareroot of a negative value should be calculated. Please check the input");
				}

				double accToConstantSpeedDuration = ((-acc*initialSpeed)-(acc*totalDuration*dec)-Math.sqrt(squared))/(acc*(acc-dec));
				...
				double breakFromConstantSpeedToZeroDuration = (-(acc*accToConstantSpeedDuration+initialSpeed))/dec;
				movement = new Movement(movement.getTrack(),accToConstantSpeedDuration,totalDuration-accToConstantSpeedDuration-breakFromConstantSpeedToZeroDuration,breakFromConstantSpeedToZeroDuration,initialSpeed,acc*accToConstantSpeedDuration+initialSpeed,0.0d);
			//if we need to speed down
			}else if(initialSpeedToDestinyDistance>totalDistance){
				double decToConstantSpeedDuration = -((2*initialSpeed*totalDuration*dec)+(initialSpeed*initialSpeed)-(2*totalDistance*dec))/(2*dec*(totalDuration*dec+initialSpeed));
				if(decToConstantSpeedDuration<0){
					throw new IllegalArgumentException("The calculated acceleration time is negative. Check input.");
				}
				double breakFromConstantSpeedToZeroDuration = (-(dec*decToConstantSpeedDuration+initialSpeed))/dec;
				movement = new Movement(movement.getTrack(),decToConstantSpeedDuration,totalDuration-decToConstantSpeedDuration-breakFromConstantSpeedToZeroDuration,breakFromConstantSpeedToZeroDuration,initialSpeed,dec*decToConstantSpeedDuration+initialSpeed,0.0d);
			//just for the case that we don't have to adjust the initial speed at all
			}else{
				movement = new Movement(movement.getTrack(),0.0d,initialSpeedDuration,breakingFromInitialSpeedToZeroDuration,initialSpeed,initialSpeed,0.0d);
			}
	 ...
	 * the deceleration and a given total duration. There's no maximum speed limitation. 
	 * @param movement The movement to be configured.
	 * @param totalDistance The total length for the movement.
	 * @param initialSpeed The initial speed.
	 * @param acc The acceleration.
	 ...
	 * deceleration of the object. Its duration represents the shortest duration to the destination possible (with the maximum speed as limitation).
	 * @param movement The movement to be configured. 
	 ...
	 */
	public static Movement configureMovement(Movement movement, double totalDistance, double initialSpeed, double maxSpeed, double acc, double dec) {
		...
		if(breakToZeroDistance<totalDistance){
			...
			if(initialSpeed<maxSpeed){
				...
				if(maxAcceleratedSpeed > maxSpeed){
					...
					movement = new Movement(movement.getTrack(), accDuration,durationWithMaxSpeed,breakingDuration,initialSpeed,maxSpeed,0.0d);
				...
				//and the breaking duration from the maximum speed to 0.
					...
					movement = new Movement(movement.getTrack(), accDuration,0.0d,breakingDuration,initialSpeed,maxSpeed,0.0d);
				...
				//and the breaking duration from the reachable speed to 0.
					...
					movement = new Movement(movement.getTrack(), accDuration,0.0d,breakingDuration,initialSpeed,maxAcceleratedSpeed,0.0d);
				}
			//If the initialSpeed lies over the maximum speed, means a break to the new maxSpeed is necessary at the beginning.
				...
				double durationWithMaxSpeed = distanceWithMaxSpeed / maxSpeed;
				movement = new Movement(movement.getTrack(), getDurationOfSpeedBoundedAcceleration(initialSpeed, maxSpeed, dec),durationWithMaxSpeed,getDurationOfSpeedBoundedAcceleration(maxSpeed, 0, dec),initialSpeed,maxSpeed,0.0d);
			}
		//If the breaking distance longer than the distance to the destination an exception will be thrown.
		...
		//If the breaking distance is exactly the distance to the destination, it'll break to zero immediately.
			...
			movement = new Movement(movement.getTrack(), 0.0d,0.0d,getDurationOfSpeedBoundedAcceleration(initialSpeed, 0.0d, dec),initialSpeed,0.0d,0.0d);
		}
	...
	public static double getCurrentSpeed(Movement movement, TimeInstant movingStartTime, TimeInstant now) {
		...
		if(timeDelta<movement.getAccDuration().getTimeAsDouble(TimeUnit.SECONDS)){
			return getSpeedOfTimedAcceleration(timeDelta, movement.getInitialSpeed(),getAcceleration(movement.getInitialSpeed(),movement.getMaxSpeed(),movement.getAccDuration()));
		//if the elapsed time is in the maximum speed phase
		}else if(timeDelta<=(movement.getAccDuration().getTimeAsDouble(TimeUnit.SECONDS)+movement.getMaxSpeedDuration().getTimeAsDouble(TimeUnit.SECONDS))){
		...
		//if the elapsed time is in the deceleration phase of the movement
			...
			double decelerationDuration = timeDelta-(movement.getAccDuration().getTimeAsDouble(TimeUnit.SECONDS)+movement.getMaxSpeedDuration().getTimeAsDouble(TimeUnit.SECONDS));
			return getSpeedOfTimedAcceleration(decelerationDuration, movement.getMaxSpeed(), getAcceleration(movement.getMaxSpeed(), movement.getEndSpeed(), movement.getDecDuration()));
	...
	public static double getDistanceOfSpeedBoundedAcceleration(double initialSpeed, double endSpeed, double acc) {
		if((endSpeed<initialSpeed&&acc>0.0d)||(endSpeed>initialSpeed&&acc<0.0d)){
			throw new IllegalArgumentException("The sign of the start and end speed don't match to the sign of the acceleration or the deceleration. The duration can't be calculated.");
		}
	 ...
	 */
	public static double getMaxAcceleratedSpeed(double distance, double initialSpeed, double endSpeed, double acc, double dec) {
		
		if(acc==dec){
			throw new IllegalArgumentException("The acceleration can't have the same value as the deceleration. Normally they have different sign.");
		}
		double  squaredSpeed = ((2 * acc * dec * distance) + (dec * initialSpeed * initialSpeed) - (acc * endSpeed * endSpeed)) / (dec - acc);
		if(squaredSpeed<0){
			throw new IllegalArgumentException("The squareroot of a negative value should be calculated. Please check the input");
		}else{
	...
	public static double getSpeedOfDistanceBoundedAcceleration(double distance, double initialSpeed, double acc){
		...
		if(speedSqr<0){
			throw new IllegalArgumentException("Invalid input. The squareroot of a negative value is tried to be calculated.");
		} 
