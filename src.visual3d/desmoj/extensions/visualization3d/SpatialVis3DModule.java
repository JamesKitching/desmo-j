...
public class SpatialVis3DModule implements VisualModule {
	 ...
	 * @param filePath The file path to the XML-file, where the model data is
	 * specified.
	 */
	...
	protected void init(final String name, String filePath) {
		...
		//set camera control
		KeyNavigatorBehavior cameraControl = new KeyNavigatorBehavior(_simpleUniverse.getViewingPlatform().getViewPlatformTransform());
...
//		rootBranch.addChild(textTG);
	...
	/**
	 * Remove the dynamic/static object from the scene graph and
	 * the map.
	...
	/**
	 * Creates a Dynamic/StaticObject according to the event. Link the model
	 * into the scene.
	 * 
	 ...
	 * @param focusOnTrack The flag indicates whether the object should facing
	 * to the moving direction while moving.
	 * @param frontSideVector The Vector which shows the the front side of the
	 * object.
	 */
