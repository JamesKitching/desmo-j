 ...
 * The translation is divided into 3 phases:</br>
 * - The acceleration phase</br>
 * - The maximum speed phase</br>
 * - The deceleration phase</br>
 * The translation behavior is defined over the speed and duration of each
 * phase.
 * @author Fred Sun
 *
 */
public class MovementPathInterpolator extends TransformInterpolator {
	...
	private Transform3D _rotTransform = new Transform3D();
	...
	/* (non-Javadoc)
	 * @see javax.media.j3d.TransformInterpolator#computeTransform(float, javax.media.j3d.Transform3D)
	 */
	@Override
	public void computeTransform(float alphaValue, Transform3D transform) {
		...
		if(alphaValue<_alphaEndAcc){
			...
			movedLength = _lengthBeginnDec + this.getDistanceOfTimedAcceleration(durationThisPhase, _speedKnots[1], _dec);
		}
	...
	/* (non-Javadoc)
	 * @see javax.media.j3d.TransformInterpolator#processStimulus(java.util.Enumeration)
	 */
	@Override
	...
	/**
	 * Updates the position based of the moved length.
	 * The directional data will also be updates if focusOnTrack is set.
	 * 
	...
	/**
	 * Calculates the length between each position knot and the start position. 
	 * The result will be stored in the length record class variable.  
	 */
	 ...
	 * @param durationKnots A float[3] array which describes the duration of the
	 * 3 phases movement.
	 * @param speedKnots A float[3] array which describes the speed of the
	 * 3 phases movement.
	 * @param positionKnots An array of points which describe the route of the
	 * movement.
	 */
