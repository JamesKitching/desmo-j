 ...
 * choice, as the <ode>WaitQueue</code> combines queueing and scheduling features).
 ...
 * reference to a <code>ProcessQueue</code> where the slaves are waiting.
 * 
...
public class WaitQueue<M extends SimProcess, S extends SimProcess> extends desmoj.core.simulator.QueueBased {
	 ...
	 * is available (order is based on the slaves' queueing priorities with 
	 * tie-breaking based on the queue's discipline as set in the constructor). 
	 * If the capacity limit of the slave queue is reached, the
