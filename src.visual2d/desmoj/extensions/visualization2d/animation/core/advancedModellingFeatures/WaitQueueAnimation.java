...
public class WaitQueueAnimation extends WaitQueue{
	...
	public boolean cooperate(ProcessCoopAnimation coop) throws SuspendExecution {
		...
		boolean	init	= this.cmdGen.isInitPhase();
		Command c;
		String[] insertMaster = {master.getName(), Integer.toString(master.getQueueingPriority()), this.masterPriorityAttribute};
		if(this.showInAnimation){
			try {
	...
	public boolean waitOnCoop() throws SuspendExecution {
		...
		boolean	init	= this.cmdGen.isInitPhase();
		Command c;
		String[] insertSlave = {slave.getName(), Integer.toString(slave.getQueueingPriority()), this.slavePriorityAttribute};
		if(this.showInAnimation){
			try {
				if(init)	c = Command.getCommandInit("setWaitQueue", this.cmdGen.getAnimationTime(simTime));
				else 		c = Command.getCommandTime("setWaitQueue", this.cmdGen.getAnimationTime(simTime));
