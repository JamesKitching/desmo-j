...
public class StockAnimation extends Stock{
	...
	public void createAnimation(Position pos, FormExt form,
	 ...
	 public boolean retrieve(long noOfProducts) throws SuspendExecution {
		...
		boolean	init	= this.cmdGen.isInitPhase();
		Command c;
		String[] retrieve = {consumer.getName(), Integer.toString(consumer.getQueueingPriority()), Long.toString(noOfProducts), this.consumerPriorityAttribute};
		if(this.showInAnimation){
			try {
	 ...
	 public boolean store(long noOfProducts) throws SuspendExecution {
		...
		boolean	init	= this.cmdGen.isInitPhase();
		Command c;
		String[] store = {producer.getName(), Integer.toString(producer.getQueueingPriority()), Long.toString(noOfProducts), this.producerPriorityAttribute};
		if(this.showInAnimation){
			try {
				if(init)	c = Command.getCommandInit("setStock", this.cmdGen.getAnimationTime(simTime));
				else 		c = Command.getCommandTime("setStock", this.cmdGen.getAnimationTime(simTime));
