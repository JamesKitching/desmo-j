...
public class URLTreePanel extends JPanel {
	 ...
	 *            experiment
     * @param appendixes
     *            List<List<String>> : the file endings of the four output channels 
	 */
	public void createNode(String nodeText, String dir, List<List<String>> appendixes) {
	    
	    String[] firstAppendixes = {appendixes.get(0).get(0), appendixes.get(1).get(0), 
	            appendixes.get(2).get(0), appendixes.get(3).get(0)};
		...
		for (int i = 0; i < treeRoot.getChildCount(); i++) {
			...
			if (nodeText.equals(treeRoot.getChildAt(i).toString())) {
				...
				appendix.add(i, firstAppendixes);
		...
		if (newExperiment) {
			...
			appendix.add(treeRoot.getChildCount(), firstAppendixes);
